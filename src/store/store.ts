...
export type StoreActions = {
  ...
  nameExists: (name: string, lists: TodoList[]) => boolean;
}
...
function isValidListName(name: string): boolean {
  return TaskNamePattern.test(name);
}


function nameExists(name: string, lists: TodoList[]): boolean {
...
export function useTodoStore(): [TodoStore, StoreActions] {
  ...
  function editListName(id: string, newName: string): boolean {
    if (!isValidListName(newName)) return false;
    if (nameExists(newName, store.lists)) return false;
